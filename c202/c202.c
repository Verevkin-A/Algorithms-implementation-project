...
void Stack_Init( Stack *stack ) {
	...
	//return error in case of NULL parameter
	if (stack == NULL) {

		Stack_Error(SERR_INIT);

	//set stack on empty value (-1)
	} else { stack->topIndex = -1; }
}
...
int Stack_IsEmpty( const Stack *stack ) {
	...
	//check if stack is empty (top index is -1)
...
int Stack_IsFull( const Stack *stack ) {
	...
	//check if top of the stack on the last index
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	//return error in case of empty stack
	if (Stack_IsEmpty(stack)) {

		Stack_Error(SERR_TOP);

	//set pointer to the value on the top index
	} else { *dataPtr = stack->array[stack->topIndex]; }
}
...
void Stack_Pop( Stack *stack ) {
	...
	//check if stack is not empty, lower index of the stack by one
...
void Stack_Push( Stack *stack, char data ) {
	...
	//return error in case of full stack
